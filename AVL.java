...
public class AVL 
{
    ...
    class NodeLevel //class to hold a node and its level in the tree
    {
        BinaryNode node; //the node
        int level; //the level in the tree
    
        NodeLevel(BinaryNode node, int level) //constructor for the class
        {
            this.node = node; //assign the node
            this.level = level; //assign the level      
        }
    }
    ...
    public void levelOrder() 
    {
        ...
        Queue<NodeLevel> queue = new LinkedList<>(); //create a queue to hold the nodes
        queue.add(new NodeLevel(root, 1)); //add the root node wrapped in the nodelevel object, starting at level 1
        int currentLevel = 1; //variable to track the current level being processed

        while (!queue.isEmpty()) //while the queue is not empty
        {
            NodeLevel nodeLevel = queue.remove(); //remove and return the head of the queue

            if (nodeLevel.level != currentLevel) //check if the node's level is different from the current level
            {
                System.out.println(); // New line for a new level
                currentLevel = nodeLevel.level; //update the current level
                System.out.print("Level " + currentLevel + ": ");  //print the new level
            } 
            else 
            {
                System.out.print(nodeLevel.level == 1 ? "Level 1: " : ", "); //If not the first node of the level, print a comma separator
            }

            System.out.print(nodeLevel.node.value); //print the value of the current node

            if (nodeLevel.node.left != null) //check if the left child of the current node exists
            {
                queue.add(new NodeLevel(nodeLevel.node.left, nodeLevel.level + 1)); //add the left child to the queue, incrementing the level by 1
            }
            if (nodeLevel.node.right != null) //check if the right child of the current node exists
            {
                queue.add(new NodeLevel(nodeLevel.node.right, nodeLevel.level + 1)); //add the right child to the queue, incrementing the level by 1
            }
        }
    }
